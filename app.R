...
# B.1. Function to Load Default Parameters
# Cloud-adjusted: no local folder is created and no .Rds file is read.
...
# Cloud-adjusted: disabled to avoid saving saved_params.Rds in a local folder.
...
# Cloud-adjusted: no local file is removed because no local file is created.
  ...
  tabsetPanel(id = "tabs",
              ...
              tabPanel("Configuration",
               ...
               margin-top: -10px; margin-bottom: 25px;"),
                           ...
                           #### C.4.1.1.4. Input: Information for Study Traceability
    ...
    tags$p("Developed by Alan Carvalho Dias |
         Powered by LabR Group  | License: GPL-3.0  | Code available at: ",
           ...
           tags$a(
             ...
             icon("github", style = "font-size: 24px;"),
             tags$span("GitHub",
                       style = "font-size: 12px;")
           ),
           ...
           tags$a(
             ...
             icon("globe", style = "font-size: 24px;"),
...
server <- function(input, output, session) {
  ...
  # D.1. Local workspace/history saving removed for cloud deployment.
  #      No .RData, .Rhistory, or local output folder is created when the app stops.
  
  
  # D.2. Reactive values for report rendering and temporary session files
  ...
  # D.3. Reactive Function to Load the File and Read Columns
  ...
  # D.5. Observer to Validate Configuration
  #      Cloud-adjusted: no configuration file is saved locally.
  
  
  observeEvent(input$save_config, {
    showModal(modalDialog(
      title = "Configuration Validated",
      "The current configuration was validated for this session. No local configuration file was created.",
      easyClose = TRUE,
      footer = NULL
    ))
  })
  ...
  observeEvent(input$clear_config, {
    ...
    showModal(modalDialog(
      ...
      "Settings cleared successfully. No local configuration file was removed because no local configuration file is created in cloud mode.",
      easyClose = TRUE,
      footer = NULL
    ))
  })
  ...
  # D.7. Observer to Generate Report with Progress Tracking
  ...
  observeEvent(input$generate_report, {
    ...
    ## D.7.1. Set session-temporary paths and params
    ##        Cloud-adjusted: no fixed output folder such as 4_Report_HTML is used.
    
    
    output_file <- tempfile(pattern = "Report_LabRI_method_", fileext = ".html")
    ...
    ## D.7.2. Setup temporary progress tracking file
    ##        This is temporary inter-process communication for callr, not a saved output.
    
    
    progress_file <- tempfile(pattern = "labri_progress_", fileext = ".log")
    ...
    ## D.7.4. Launch background R process for rendering
    
    
    rmd_path <- normalizePath("LabRI_script_connect.Rmd")
  ...
  ## D.7.5. Poll progress file and update modal
  ...
  observe({
    ...
    if (finished) {
      ...
      output_file <- isolate(rv$output_file)
      
      output$report_content <- renderUI({
        req(output_file)
        req(file.exists(output_file))
        div(
          style = "width: 100%;max-height: 900px;overflow-y: auto;border: none;",
      ...
      showModal(modalDialog(
        ...
        "The report was generated successfully and is available for viewing in the 'Report' tab. No persistent HTML file was exported to an app folder.",
        easyClose = FALSE,
        footer = tagList(
          modalButton("Close"),
      ...
      ##########################################################################
      ## Friendly error message for memory limitation in cloud environments #####
      ##########################################################################
      
      
      is_memory_error <- grepl(
        "error 137|pandoc document conversion failed with error 137|Killed|SIGKILL|cannot allocate vector|std::bad_alloc|memory",
        error_msg,
        ignore.case = TRUE
      )
      
      
      error_title <- if (is_memory_error) {
      ...
      friendly_error_msg <- if (is_memory_error) {
        
        HTML("
        <div style='text-align: justify; line-height: 1.6; color: black;'>
          
          <p><strong>Memory limit reached during report generation.</strong></p>
          
          <p>
            The LabRI analysis was processed, but the cloud server stopped the
            final HTML report conversion because the available memory was not
            sufficient.
          </p>
          
          <p>
            This usually occurs when the dataset is large or when the report
            contains many figures, interactive plots, large tables, or embedded
            HTML components. In free cloud environments, the available memory is
            limited, and the final report-rendering step may require more memory
            than the statistical calculations themselves.
          </p>
          
          <p>
            This does not necessarily indicate a problem with the dataset or with
            the LabRI method.
          </p>
          ...
          <ul>
            <li>Reduce the maximum sample size and run the analysis again.</li>
            <li>Use a smaller dataset for cloud demonstration purposes.</li>
            <li>Use the local/zipped version of LabRI for larger analyses.</li>
            <li>Use a Posit Connect environment with more memory for larger datasets.</li>
          </ul>
          
          <hr>
          
          <p style='font-size: 12px; color: #666;'>
            Technical detail: Pandoc document conversion failed because the
            process was likely stopped by the operating system due to
            insufficient memory.
          </p>
  ...
  ## D.7.6. Observer to Cancel the Report Generation
