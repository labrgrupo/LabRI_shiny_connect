...
###### 3.1.1.5.1. Initialize trace log ######################################### 
...
###### 3.1.1.5.3. Tolerant validator (returns c(NA, NA) instead of stopping) ### 

  
  .validate_ri_tolerant <- function(x, name) {

    
####### 3.1.1.5.3.1. Case 1: NULL -> treated as absent
...
####### 3.1.1.5.3.2. Case 2: wrong length -> treated as absent (logged)
...
####### 3.1.1.5.3.3. Case 3: coerce to numeric
...
####### 3.1.1.5.3.4. Case 4: NaN -> normalize to NA
...
####### 3.1.1.5.3.5. Case 5: Inf/-Inf -> normalize to NA (logged)
...
####### 3.1.1.5.3.6. Case 6: LRL > URL -> treated as malformed, normalized to NA
...
###### 3.1.1.5.4. Validate all three RIs (tolerant) ############################
...
###### 3.1.1.5.5. Decision rule ################################################ 

  
  if (isTRUE(NP_method)) {

####### 3.1.1.5.5.1. Branch: NP_method = TRUE (low-count scenario)
...
####### 3.1.1.5.5.2. Branch: NP_method = FALSE (standard scenario)
...
###### 3.1.1.5.6. Return structured output ##################################### 

  list(
    selected_RI     = selected_RI,
    selected_method = selected_method,

####### 3.1.1.5.6.1. Original (normalized) RIs preserved for audit

    refineR_RI      = ri_refineR,
    reflimR_RI      = ri_reflimR,
    NP_RI           = ri_NP,

####### 3.1.1.5.6.2. Failure flags

    refineR_failed  = refineR_failed,
    reflimR_failed  = reflimR_failed,
    NP_failed       = NP_failed,

####### 3.1.1.5.6.3. Full traceability
...
titulo.fig.3<-paste("Fig. 3. Q-Q plot before outlier exclusion (n = ",sample.size.select," ).")
...
###### 3.1.1.12.1. Reference limits NP LabRI low-count (Before) ################
...
titulo.Fig.4 <- paste("Fig. 4 and 5. LabRI reference interval under a low-count scenario before data cleaning (replaces the previous refineR and reflimR figures; n =",sample.size.select, ")")
...
###### 3.1.1.12.2. Reference Interval refineR (before) #########################
...
refineR.LabRI.1<-findRI_modified(resultados,
                                 nThreshold = 10000,
...
refineR.cenario.1<-refineR::getRI(refineR.LabRI.1,
...
Fig.4.refineR<-plot(refineR.LabRI.1,
                    Scale = "original",
                    RIperc = RIperc2,
                    showPathol = TRUE,
                    scalePathol = TRUE,
...
###### 3.1.1.12.3. Reference Interval reflimR (before) #########################


####### 3.1.1.12.3.1. If the side is Right_sided
...
######## 3.1.1.12.3.1.1. Statistical parameters (mean and SD) reflimR
...
######## 3.1.1.12.3.1.2. Reference limits reflimR
...
######## 3.1.1.12.3.1.3. reflimR Histogram

    
lambda.reflimR.1<-ifelse(isTRUE(reflimR::lognorm(resultados,
...
####### 3.1.1.12.3.2. If the side is Two_sided
...
######## 3.1.1.12.3.2.1. Statistical parameters (mean and SD) reflimR
    
    meanlog.reflim<-IR.reflim$stats[1]
    sdlog.reflim<-IR.reflim$stats[2]
    
######## 3.1.1.12.3.2.2. Reference limits reflimR
...
######## 3.1.1.12.3.2.3. reflimR Histogram
...
val.LRL.RI.refineR.cenario.1<-ifelse(isTRUE(use.np.method),NA,
                                            LRL.RI.refineR.cenario.1)
val.URL.RI.refineR.cenario.1<-ifelse(isTRUE(use.np.method),NA,
                                            URL.RI.refineR.cenario.1)

val.LRL.RI.reflimR.cenario.1<-ifelse(isTRUE(use.np.method),NA,
                                            LRL.RI.reflimR.cenario.1)
val.URL.RI.reflimR.cenario.1<-ifelse(isTRUE(use.np.method),NA,
                                            URL.RI.reflimR.cenario.1)

val.LRL.RI.LabRI.NP.method.low.count.cenario.1<-ifelse(isTRUE(use.np.method),
                                                       LRL.RI.LabRI.NP.method.low.count.cenario.1,
                                            NA)
val.URL.RI.LabRI.NP.method.low.count.cenario.1<-ifelse(isTRUE(use.np.method),
                                                       URL.RI.LabRI.NP.method.low.count.cenario.1,
                                                       NA
                                            )


RI.fallback.1<-select_RI_by_scenario(ri_refineR=c(val.LRL.RI.refineR.cenario.1,
                                                  val.URL.RI.refineR.cenario.1),
                                     ri_reflimR = c(val.LRL.RI.reflimR.cenario.1,
                                                    val.URL.RI.reflimR.cenario.1),
                                     ri_NP = c(val.LRL.RI.LabRI.NP.method.low.count.cenario.1,
...
if (nrow(dataset.escolhido) > n_bic_max) {
  ...
  message(sprintf("[LabRI] mclust BIC: using subsample of %d (original n = %d)",
                  n_bic_max, nrow(dataset.escolhido)))
...
hist.mix <- plot.hist.mix(fit.1.his, family = family.np.method)
