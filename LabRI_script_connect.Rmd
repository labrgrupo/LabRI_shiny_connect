...
# Note 8: The default number of decimal places is inferred from the dataset..
...
casa.decimal <- if (isTRUE(interpretando.casa.decimal)) {
  ...
  as.numeric(Number_of_decimal_places)
}
...
###### 3.1.3.1.3.1. If LI < 0 => 0
      
      if (is.finite(LI) && LI < 0) LI <- 0
      
###### 3.1.3.1.3.2. Respects the lowest value
      
      LI <- max(LI, 0, menor_valor, na.rm = TRUE)
      
###### 3.1.3.1.3.3. If LI == LS
      
      if (is.finite(LI) && is.finite(LS) && LI == LS) {
        LI <- max(0, menor_valor)
      }
      
###### 3.1.3.1.3.4. Right one-tailed test
      
      if (lado == "Right_sided") {
        LI <- max(0, menor_valor)
      }
      
###### 3.1.3.1.3.5. Stabilization check
...
###### 3.1.3.1.3.7. Reliable statistics
...
###### 3.1.3.2.1. iBoxplot Algorithm and Box_Plot
...
###### 3.1.3.2.2. First filtering of the original dataset
...
###### 3.1.3.2.3. Box-Cox transf and Pruning Data Tails for Bowley's Sk   
      ...
      if (!requireNamespace("forecast", quietly = TRUE)) {
        ...
        Box.Cox.Decision <- ifelse(
          ...
          ifelse(
            ...
            FALSE,
            ifelse(
              is.finite(sk_transf) && abs(sk_transf) < abs(sk_orig) && delta > 0.05,
              TRUE,
              FALSE
            )
          )
...
###### 3.1.3.2.4. Lognormality decision and Dataset Select
    
    tail_pruning_results <- Adaptive_Tail_Pruning(
    ...
    ds_orig_PruneTails <- tail_pruning_results$ds_orig_PruneTails
    sk_orig <- tail_pruning_results$sk_orig
    sk_transf <- tail_pruning_results$sk_transf
    delta <- tail_pruning_results$delta_skew
    Box.Cox.Decision <- tail_pruning_results$boxcox_decision
    sk.Pearson <- tail_pruning_results$sk_Pearson
...
###### 3.1.3.2.5. List of outputs from the preproc function
...
Table.6 <- kbl(
  ...
  row_spec(
    ...
    color = "black",
    background = ifelse(isTRUE(needs_log_transform), "gold", "palegreen")
  ) |>
  column_spec(
    1,
    bold = TRUE,
...
################################################################################
##### 3.1.1.2. Adapted reflimR histogram function ##############################
################################################################################
...
###### 3.1.1.2.2. Margin and Layout Adjustment #################################
  
  oldpar <- par(no.readonly = TRUE)
  on.exit(par(oldpar), add = TRUE)
  par(mar = c(5, 4, 4, 2) + 0.1, mgp = c(3, 1, 0))
  
###### 3.1.1.2.3. xlim "smart" ################################################
  
  difference <- max(xx) - min(xx)
  x_min_base <- min(xx) - 0.1 * difference
  
  if (!is.finite(x_min_base) || x_min_base < 0) {
    x_min_base <- 0
  }
  ...
  if (!is.finite(plot_w_in) || plot_w_in <= 0) {
    plot_w_in <- 7
  }
...
###### 3.1.1.2.8. Reference Limits reflimR #####################################
...
################################################################################
##### 3.1.1.3. LabRI nonparametric RI function for a low-count scenario ########
################################################################################
...
###### 3.1.1.3.1. Extract the column with the data #############################
  
  stopifnot(is.data.frame(dataset))
  
  if (!col_valor %in% names(dataset)) {
    stop("The column '", col_valor, "' it does not exist in the 'dataset' dataframe.")
  }
  ...
  if (length(x) == 0) {
    stop("There is no valid (numeric) data in dataset[['", col_valor, "']].")
  }
  
###### 3.1.1.3.2. Calculates IR non-parametric limits ##########################
  
  if (lado == "Two_sided") {
    LRL <- as.numeric(quantile(x, probs = 0.025, na.rm = na.rm, names = names, type = type))
    URL <- as.numeric(quantile(x, probs = 0.975, na.rm = na.rm, names = names, type = type))
  } else {
    LRL <- 0
    URL <- as.numeric(quantile(x, probs = 0.95, na.rm = na.rm, names = names, type = type))
  }
...
################################################################################
##### 3.1.1.4. Fallback RI selection function ##################################
################################################################################
...
select_RI_by_scenario <- function(ri_refineR = NULL,
  ...
  if (!is.logical(NP_method) || length(NP_method) != 1L || is.na(NP_method)) {
    stop("NP_method must be a single TRUE/FALSE value (got: ", deparse(NP_method), ").")
  }
  
  .validate_ri_tolerant <- function(x, name) {
    
    if (is.null(x)) {
      .log_msg("warnings", sprintf("%s is NULL (treated as absent).", name))
    ...
    if (length(x) != 2L) {
      .log_msg(
        "warnings",
        sprintf("%s has length %d (expected 2). Treated as absent.", name, length(x))
      )
      return(c(NA_real_, NA_real_))
    }
    ...
    if (!is.numeric(x)) {
      ...
      if (anyNA(x) && !anyNA(x_orig)) {
        .log_msg(
          "warnings",
          sprintf("%s coerced to numeric introduced NA (original: %s).", name, deparse(x_orig))
        )
      }
    }
    
    if (any(is.nan(x))) {
      .log_msg("warnings", sprintf("%s contains NaN (normalized to NA).", name))
      x[is.nan(x)] <- NA_real_
    }
    
    if (any(is.infinite(x))) {
      .log_msg("warnings", sprintf("%s contains Inf/-Inf (normalized to NA).", name))
    ...
    if (all(!is.na(x)) && x[1] > x[2]) {
      .log_msg(
        "warnings",
        sprintf("%s has LRL > URL (LRL=%g, URL=%g). Treated as absent.", name, x[1], x[2])
      )
      return(c(NA_real_, NA_real_))
    }
  ...
  if (isTRUE(NP_method)) {
    ...
    if (!refineR_failed) {
      ...
      .log_msg("warnings", "refineR failed; falling back to reflimR.")
      ...
      .log_msg("errors", "Both refineR and reflimR failed. RI cannot be estimated.")
...
################################################################################
##### 3.1.1.5. Histogram function for LabRI NP RI in a low-count scenario ######
################################################################################
...
hist_LabRI.NP.low.count <- function(x,
  ...
  if (length(x) < 2) {
    stop("x must have at least 2 non-NA values.")
  }
  ...
  if (!is.null(limits)) {
    ...
    if (anyNA(limits)) {
      stop("limits contains NA.")
    }
    
    if (limits[1] > limits[2]) {
      stop("limits has LRL > URL.")
    }
...
titulo.fig.1 <- paste(
  "Fig. 1. Density plot and boxplot before outlier exclusion (n = ",
  sample.size.select, " )."
)
...
P.g2 <- qplot(
  ...
  stat_function(
    fun = function(x) dnorm(x, mean = P.media, sd = P.dp),
    color = "black",
    size = 1.2
  ) +
  geom_vline(aes(xintercept = P.moda, colour = P.moda.texto), size = 1.2, linetype = "solid") +
  geom_vline(aes(xintercept = P.mediana, colour = P.mediana.texto), size = 1.2, linetype = "solid") +
  geom_vline(aes(xintercept = P.media, colour = P.media.texto), size = 1.2, linetype = "solid") +
  scale_colour_manual(
    "",
    breaks = c(P.moda.texto, P.mediana.texto, P.media.texto),
...
Fig.1 <- ggarrange(P.g2, P.g1, heights = c(3, 2), align = "hv", ncol = 1, nrow = 2)

Fig.1


titulo.fig.2 <- paste(
  "Fig. 2. Ogive graph before detection and exclusion of outliers (n = ",
  sample.size.select, " )."
)
...
################################################################################
##### 3.1.1.7. Analyze Kurtosis ################################################
################################################################################


P.Kurtosis <- round(moments::kurtosis(resultados), 2)

P.conclusao.Kurtosis <- ifelse(
  P.Kurtosis > 3.3,
  Leptokurtic,
...
################################################################################
##### 3.1.1.8. Asymmetry Analysis ##############################################
################################################################################


P.ordem.neg.assimetria <- ifelse(
  P.moda > P.mediana & P.moda > P.media & P.mediana > P.media,
  "ok",
  "nao ok"
)

P.ordem.pos.assimetria <- ifelse(
  P.media > P.mediana & P.media > P.moda & P.mediana > P.moda,
  "ok",
  "nao ok"
)
...
################################################################################
##### 3.1.1.9. Quantil-Quantil Plot ############################################
################################################################################


titulo.fig.3 <- paste(
  "Fig. 3. Q-Q plot before outlier exclusion (n = ",
  sample.size.select, " )."
)
...
################################################################################
##### 3.1.1.11. Decision: refineR-reflimR or LabRI NP low-count before #########
################################################################################


if (isTRUE(use.np.method)) {
  
###### 3.1.1.11.1. Reference limits NP LabRI low-count before ##################
  ...
  titulo.Fig.4 <- paste(
    "Fig. 4 and 5. LabRI reference interval under a low-count scenario before data cleaning",
    "(replaces the previous refineR and reflimR figures; n =",
    sample.size.select,
    ")"
  )
...
###### 3.1.1.11.2. Reference Interval refineR before ###########################
  ...
  URL.RI.refineR.cenario.1 <- round(refineR.cenario.1$PointEst[2], digits = casa.decimal)
  LRL.RI.refineR.cenario.1 <- round(refineR.cenario.1$PointEst[1], digits = casa.decimal)
...
###### 3.1.1.11.3. Reference Interval reflimR before ###########################
...
################################################################################
##### 3.1.1.12. Fallback RI selection among NP, refineR, and reflimR before ####
################################################################################
...
S.media <- round(mean(dataset_orig_truncado$dataset_orig_truncado),
                 digits = casa.decimal2)

S.mediana <- round(median(dataset_orig_truncado$dataset_orig_truncado),
                   digits = casa.decimal2)
...
titulo.fig.6 <- paste(
  "Fig. 6. Density plot and box plot after outlier exclusion (n<sub>New1</sub> = ",
  n.after.truncation,
  " )."
)


S.dp <- round(sd(dataset_orig_truncado$dataset_orig_truncado),
              digits = casa.decimal2)


S.g1 <- qplot(
  x = "",
  y = dataset_orig_truncado,
...
S.g2 <- qplot(
  ...
  geom_density(color = "sienna3", alpha = 0.6, fill = "sienna1") +
  stat_function(
    fun = function(x) dnorm(x, mean = S.media, sd = S.dp),
    color = "black",
    size = 1.2
  ) +
  geom_vline(
    aes(xintercept = S.moda, colour = S.moda.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  geom_vline(
    aes(xintercept = S.mediana, colour = S.mediana.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  geom_vline(
    aes(xintercept = S.media, colour = S.media.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  scale_colour_manual(
    "",
    breaks = c(S.moda.texto, S.mediana.texto, S.media.texto),
    values = c("red", "springgreen4", "blue2")
  ) +
  theme(
    legend.position = "top",
...
titulo.fig.7 <- paste(
  "Fig. 7. Ogive graph after outlier exclusion (n<sub>New1</sub> = ",
  n.after.truncation,
  " )."
)
...
S.Kurtosis <- round(
  moments::kurtosis(dataset_orig_truncado$dataset_orig_truncado),
  2
)


S.conclusao.Kurtosis <- ifelse(
  S.Kurtosis > 3.3,
  Leptokurtic,
  ifelse(
    S.Kurtosis < 2.7,
...
S.analise.cor.Kurtosis <- ifelse(
  S.conclusao.Kurtosis == Mesokurtic,
  "ok",
  "nao ok"
)
...
S.ordem.neg.assimetria <- ifelse(
  S.moda > S.mediana & S.moda > S.media & S.mediana > S.media,
  "ok",
  "nao ok"
)


S.ordem.pos.assimetria <- ifelse(
  S.media > S.mediana & S.media > S.moda & S.mediana > S.moda,
  "ok",
  "nao ok"
)


S.conclusao.assimetria.1 <- ifelse(
  S.ordem.pos.assimetria == "ok" & S.ordem.neg.assimetria == "nao ok",
  "Empirical relationship between Mean, Median, and Mode is: Mean > Median > Mode. ",
  ifelse(
    S.ordem.pos.assimetria == "nao ok" & S.ordem.neg.assimetria == "ok",
...
S.assimetria <- signif(
  round(skewness(dataset_orig_truncado$dataset_orig_truncado), digits = 2),
  4
)


S.magnitude.assimetria <- ifelse(
  abs(S.assimetria) > 1,
  "The distribution is highly skewed",
  ifelse(
    abs(S.assimetria) > 0.5 & abs(S.assimetria) <= 1,
    "The distribution is moderately skewed",
    ifelse(
      abs(S.assimetria) > 0.15 & abs(S.assimetria) <= 0.5,
      "The distribution is slightly skewed",
...
S.conclusao.assimetria <- paste(
  S.conclusao.assimetria.1,
  S.conclusao.assimetria.2
)


S.analise.cor.assimetria <- ifelse(
  S.magnitude.assimetria == "The distribution is highly skewed",
  "nao ok",
  "ok"
)
...
titulo.fig.8 <- paste(
  "Fig. 8. Q-Q plot after outlier exclusion (n<sub>New1</sub> = ",
  n.after.truncation,
  " )."
)
...
Table.8.data.frame <- data.frame(
  S.lista.nomes.shape,
  S.lista.resultados.shape
)


titulo.tabela.8 <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 8.</strong> Data shape after outlier detection (n<sub>New1</sub> = ",
  n.after.truncation,
  ").</div>"
)
...
Table.8 <- kbl(
  ...
  row_spec(
    ...
    color = "black",
    background = ifelse(
      is.na(S.analise.cor.Kurtosis) == TRUE,
      "gold",
      ifelse(S.analise.cor.Kurtosis == "ok", "palegreen", "gold")
    )
  ) |>
  row_spec(
    7,
    ...
    color = "black",
    background = ifelse(
      is.na(S.analise.cor.assimetria) == TRUE,
      "gold",
      ifelse(S.analise.cor.assimetria == "ok", "palegreen", "gold")
    )
  ) |>
  column_spec(
    1,
...
###### 3.1.1.8.1. Reference limits NP LabRI low-count After ####################
  ...
  LabRI.NP.method.low.count.2 <- RI_LabRI_np(
  ...
  LabRI.NP.low.count.cenario.2 <- paste(
    round(LRL.RI.LabRI.NP.method.low.count.cenario.2, digits = casa.decimal),
    "to",
    round(URL.RI.LabRI.NP.method.low.count.cenario.2, digits = casa.decimal)
  )
  
  
  titulo.Fig.9 <- paste(
    "Fig. 9 and 10. LabRI reference interval under a low-count scenario after data cleaning (replaces the previous refineR and reflimR figures; n =",
    n.after.truncation,
    ")"
  )
  
  
  Fig.9.LabRI <- hist_LabRI.NP.low.count(
    x = dataset_orig_truncado$dataset_orig_truncado,
    n_breaks = n.break,
    main = titulo.Fig.9,
    xlab = mensurando,
    ylab = "Frequency",
...
###### 3.1.1.8.2. Reference Interval refineR After #############################
  ...
  refineR.cenario.2 <- refineR::getRI(
    refineR.2,
    pointEst = "fullDataEst",
    RIperc = RIperc
  )
  ...
  LRL.RI.refineR.cenario.2 <- ifelse(
    LRL.RI.refineR.cenario.2 < 0.0001,
    0,
...
###### 3.1.1.8.3. Reference Interval reflimR after #############################
  ...
  if (lado == "Right_sided") {
    ...
    URL.RI.reflimR.cenario.2 <- ifelse(
      needs_log_transform == TRUE,
    ...
    IR.reflimR.cenario.2 <- paste(
      round(LRL.RI.reflimR.cenario.2, digits = casa.decimal),
      "to",
      round(URL.RI.reflimR.cenario.2, digits = casa.decimal)
    )
    
    
    lambda.reflimR.2 <- ifelse(
      isTRUE(reflimR::lognorm(
        dataset_orig_truncado$dataset_orig_truncado,
        plot.it = FALSE
      )$lognormal),
      0,
...
val.LRL.RI.refineR.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  NA,
  LRL.RI.refineR.cenario.2
)
...
val.URL.RI.refineR.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  NA,
...
val.LRL.RI.reflimR.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  NA,
  LRL.RI.reflimR.cenario.2
)
...
val.URL.RI.reflimR.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  NA,
  URL.RI.reflimR.cenario.2
)
...
val.LRL.RI.LabRI.NP.method.low.count.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  LRL.RI.LabRI.NP.method.low.count.cenario.2,
  NA
)
...
val.URL.RI.LabRI.NP.method.low.count.cenario.2 <- ifelse(
  isTRUE(use.np.method),
  URL.RI.LabRI.NP.method.low.count.cenario.2,
  NA
)
...
RI.fallback.2 <- select_RI_by_scenario(
  ri_refineR = c(
    val.LRL.RI.refineR.cenario.2,
    val.URL.RI.refineR.cenario.2
  ),
  ri_reflimR = c(
    val.LRL.RI.reflimR.cenario.2,
    val.URL.RI.reflimR.cenario.2
  ),
  ri_NP = c(
    val.LRL.RI.LabRI.NP.method.low.count.cenario.2,
...
LRL.refineR.reflimR.NP.fallback.cenario.2 <- round(
  RI.fallback.2$selected_RI[1],
  digits = casa.decimal
)


URL.refineR.reflimR.NP.fallback.cenario.2 <- round(
  RI.fallback.2$selected_RI[2],
  digits = casa.decimal
)


IR.refineR.reflimR.NP.fallback.cenario.2 <- paste(
...
freq.rel.dif.1.para.2 <- ifelse(is.na(freq.rel.dif.1.para.2) == TRUE, 1, freq.rel.dif.1.para.2)
freq.rel.dif.2.para.3 <- ifelse(is.na(freq.rel.dif.2.para.3) == TRUE, 1, freq.rel.dif.2.para.3)
freq.rel.dif.3.para.4 <- ifelse(is.na(freq.rel.dif.3.para.4) == TRUE, 1, freq.rel.dif.3.para.4)
freq.rel.dif.4.para.5 <- ifelse(is.na(freq.rel.dif.4.para.5) == TRUE, 1, freq.rel.dif.4.para.5)
freq.rel.dif.5.para.6 <- ifelse(is.na(freq.rel.dif.5.para.6) == TRUE, 1, freq.rel.dif.5.para.6)
freq.rel.dif.6.para.7 <- ifelse(is.na(freq.rel.dif.6.para.7) == TRUE, 1, freq.rel.dif.6.para.7)
...
freq.acum.dif.1.para.6 <- freq.rel.dif.1.para.2 +
  freq.rel.dif.2.para.3 +
  freq.rel.dif.3.para.4 +
  freq.rel.dif.4.para.5 +
...
freq.acum.dif.1.para.7 <- freq.rel.dif.1.para.2 +
  freq.rel.dif.2.para.3 +
  freq.rel.dif.3.para.4 +
  freq.rel.dif.4.para.5 +
  freq.rel.dif.5.para.6 +
...
vetor.k.2 <- c(
  freq.acum.dif.1.para.2,
  freq.acum.dif.1.para.3,
  freq.acum.dif.1.para.4,
  freq.acum.dif.1.para.5,
  freq.acum.dif.1.para.6,
  freq.acum.dif.1.para.7
)
...
soma.dif.11.ate.77 <- dif.11.para.22 +
  dif.22.para.33 +
  dif.33.para.44 +
  dif.44.para.55 +
  dif.55.para.66 +
...
freq.rel.dif.11.para.22 <- ifelse(is.na(freq.rel.dif.11.para.22) == TRUE, 1, freq.rel.dif.11.para.22)
freq.rel.dif.22.para.33 <- ifelse(is.na(freq.rel.dif.22.para.33) == TRUE, 1, freq.rel.dif.22.para.33)
freq.rel.dif.33.para.44 <- ifelse(is.na(freq.rel.dif.33.para.44) == TRUE, 1, freq.rel.dif.33.para.44)
freq.rel.dif.44.para.55 <- ifelse(is.na(freq.rel.dif.44.para.55) == TRUE, 1, freq.rel.dif.44.para.55)
freq.rel.dif.55.para.66 <- ifelse(is.na(freq.rel.dif.55.para.66) == TRUE, 1, freq.rel.dif.55.para.66)
freq.rel.dif.66.para.77 <- ifelse(is.na(freq.rel.dif.66.para.77) == TRUE, 1, freq.rel.dif.66.para.77)
...
freq.acum.dif.11.para.66 <- freq.rel.dif.11.para.22 +
  freq.rel.dif.22.para.33 +
  freq.rel.dif.33.para.44 +
  freq.rel.dif.44.para.55 +
...
freq.acum.dif.11.para.77 <- freq.rel.dif.11.para.22 +
  freq.rel.dif.22.para.33 +
  freq.rel.dif.33.para.44 +
  freq.rel.dif.44.para.55 +
  freq.rel.dif.55.para.66 +
...
vetor.k.22 <- c(
  freq.acum.dif.11.para.22,
  freq.acum.dif.11.para.33,
  freq.acum.dif.11.para.44,
  freq.acum.dif.11.para.55,
  freq.acum.dif.11.para.66,
  freq.acum.dif.11.para.77
)
...
n.modas.1 <- nmodes(
  data = dataset.escolhido$dataset.escolhido,
...
k.new <- select_k_9criteria(
  ...
  crit9 = isTRUE(use.np.method == TRUE)
)$crit_conclusion
...
lista.criterios.n.clusters <- c(
  "Estimated the number of clusters by counting modes in a Gaussian-kernel KDE, using a 1.06 normal-reference bandwidth constant",
...
Table.10.data.frame <- data.frame(
  lista.criterios.n,
...
titulo.tabela.10 <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 10.</strong> Multicriteria cluster-number estimates and final consensus selection (nonparametric KDE modes, Gaussian-mixture BIC criteria, and low-count override) (\u03BB<sub>LabRI</sub> = 1; n<sub>New1</sub> =",
  n.after.truncation,
  ").</div>"
)


titulo.tabela.10.transf <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 10.</strong> Multicriteria cluster-number estimates and final consensus selection (nonparametric KDE modes, Gaussian-mixture BIC criteria, and low-count override) (",
  lambda.method,
  "; n<sub>New1</sub> =",
  n.after.truncation,
  ").</div>"
...
Table.10 <- kbl(
  ...
  footnote(
    general_title = "<div style='text-align: justify; color: black;'>Footnote</div>",
    general = footnote_text.Tables.10,
    escape = FALSE
  )
...
fig.14 <- ggplot(
  ...
  scale_x_continuous(
    labels = as.character(lista.n.componentes),
  ...
  geom_line(size = 1.5) +
  labs(
    title = if (needs_log_transform) titulo.fig.14.transf else titulo.fig.14,
...
lista.modas <- c(
  ifelse(is.na(moda.1) == TRUE, 0, moda.1),
...
decidir_ev_mixfit <- function(dados,
                              ncomp,
                              mu_inicial,
                              family = "normal",
                              tol = 1e-10,
...
###### 3.2.9.1.2. Definition of initial mean (mu0)
    ...
    if (length(mu0) != ncomp) {
      mu0 <- as.numeric(quantile(
        x,
        probs = seq(0.1, 0.9, length.out = ncomp),
        na.rm = TRUE,
        names = FALSE
      ))
    }
...
###### 3.2.9.1.4. Robust log-likelihood extractor
...
##### 3.2.9.2. Adjust the two candidates: ev=TRUE and ev=FALSE #################
    
    fit_eq <- fit_safe(TRUE)
    fit_neq <- fit_safe(FALSE)
    
###### 3.2.9.2.1. If both fail, assume ev=TRUE
...
family.np.method <- ifelse(isTRUE(use.np.method), "lnorm", "normal")
...
fit.1 <- mixfit(
  ...
  tol = 1e-18,
  max_iter = 10000,
  family = family.np.method,
  mu = remover_zeros(lista.modas),
  ev = ev
)
...
######## 3.2.10.2.2.3.4. Unification Decision Logic
...
###### 3.2.10.2.3. Definition of the Unified Group and Truncation Limits
  ...
  unified_indices <- which(
    adjacency_df$unify_with_dominant &
...
###### 3.2.10.2.4. Individual Limits and Summary
  ...
  if (!is.null(lower_floor) && is.finite(lower_floor)) {
    individual_limits$LL_individual <- pmax(
      individual_limits$LL_individual,
      lower_floor
    )
  }
...
####### 3.2.10.2.4.1. Criteria summary
  ...
  criteria_summary <- adjacency_df[
    adjacency_df$adjacent_to_dominant &
      adjacency_df$valid,
  ]
  
  if (nrow(criteria_summary) > 0) {
    criteria_summary <- criteria_summary[, c(
      "comp_raw",
      "mu",
      "pi",
      "BC",
...
###### 3.2.10.2.5. Return Results
  ...
  list(
    ...
    individual_limits = individual_limits[, c(
      "comp_raw",
      "mu",
      "sd",
      "pi",
...
range.truncado.cluster.1 <- paste(
  round(LI.Cluster.1, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.1 <- paste(
  round(LI.IR.Cluster.1, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.1, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.2 <- paste(
  round(LI.Cluster.2, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.2 <- paste(
  round(LI.IR.Cluster.2, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.2, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.3 <- paste(
  round(LI.Cluster.3, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.3 <- paste(
  round(LI.IR.Cluster.3, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.3, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.4 <- paste(
  round(LI.Cluster.4, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.4 <- paste(
  round(LI.IR.Cluster.4, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.4, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.5 <- paste(
  round(LI.Cluster.5, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.5 <- paste(
  round(LI.IR.Cluster.5, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.5, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.6 <- paste(
  round(LI.Cluster.6, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.6 <- paste(
  round(LI.IR.Cluster.6, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.6, digits = ajuste.casa.decimal)
)
...
range.truncado.cluster.7 <- paste(
  round(LI.Cluster.7, digits = ajuste.casa.decimal),
  " to ",
...
IR.cluster.7 <- paste(
  round(LI.IR.Cluster.7, digits = ajuste.casa.decimal),
  " to ",
  round(LS.IR.Cluster.7, digits = ajuste.casa.decimal)
)
...
decisao.cluster.1 <- ifelse(
  Mi.1 > LI.Truncado.cluster.mae & Mi.1 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.2 <- ifelse(
  Mi.2 > LI.Truncado.cluster.mae & Mi.2 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.3 <- ifelse(
  Mi.3 > LI.Truncado.cluster.mae & Mi.3 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.4 <- ifelse(
  Mi.4 > LI.Truncado.cluster.mae & Mi.4 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.5 <- ifelse(
  Mi.5 > LI.Truncado.cluster.mae & Mi.5 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.6 <- ifelse(
  Mi.6 > LI.Truncado.cluster.mae & Mi.6 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)


decisao.cluster.7 <- ifelse(
  Mi.7 > LI.Truncado.cluster.mae & Mi.7 < LS.Truncado.cluster.mae,
  "incluir",
  "nao incluir"
)
...
cor.coluna.table.4 <- c(
  ifelse(decisao.cluster.1 == "incluir", ifelse(k.new >= 1, 2, ""), ""),
...
LS.Truncado <- ifelse(
  LS.Truncado >= max(dataset.escolhido$dataset.escolhido),
...
LI.Truncado <- ifelse(
  LI.Truncado <= min(dataset.escolhido$dataset.escolhido),
...
range.truncado.clusters <- paste(
  round(LI.Truncado, digits = ajuste.casa.decimal),
  " to ",
  round(LS.Truncado, digits = ajuste.casa.decimal)
)
...
lista.antimodas <- c(
  ifelse(is.na(round(anti.moda.1, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.1, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(anti.moda.2, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.2, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(anti.moda.3, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.3, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(anti.moda.4, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.4, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(anti.moda.5, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.5, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(anti.moda.6, digits = ajuste.casa.decimal)) == TRUE, "--", round(anti.moda.6, digits = ajuste.casa.decimal)),
  "--"
)


lista.modas.2 <- c(
  ifelse(is.na(round(moda.1, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.1, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.2, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.2, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.3, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.3, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.4, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.4, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.5, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.5, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.6, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.6, digits = ajuste.casa.decimal)),
  ifelse(is.na(round(moda.7, digits = ajuste.casa.decimal)) == TRUE, "--", round(moda.7, digits = ajuste.casa.decimal))
)
...
titulo.tabela.11 <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 11.</strong> Modes and anti-modes (\u03BB<sub>LabRI</sub> = 1; n<sub>New1</sub> =",
  n.after.truncation,
  "; bw<sub>critical</sub> = ",
  bw.critical,
  ").</div>"
...
titulo.tabela.11.transf <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 11.</strong> Modes and anti-modes (",
  lambda.method,
  "; n<sub>New1</sub> =",
  n.after.truncation,
  "; bw<sub>critical</sub> = ",
...
Table.11.data.frame <- data.frame(
  lista.n.moda.antimoda,
  lista.modas.2,
  lista.antimodas
)
...
Table.11 <- kbl(
  ...
  footnote(
    general_title = "<div style='text-align: justify; color: black;'>Footnote</div>",
    general = footnote_text.Tables.11,
    escape = FALSE
  )
...
Q.media <- round(
  mean(dataset.truncado$dataset.truncado),
  digits = ajuste.casa.decimal
)

Q.mediana <- round(
  median(dataset.truncado$dataset.truncado),
  digits = ajuste.casa.decimal
)


Q.midpoint <- ifelse(
  Q.media > Q.mediana & Q.media > Q.moda & Q.mediana > Q.moda,
  Q.moda,
  ifelse(
    Q.moda > Q.mediana & Q.moda > Q.media & Q.mediana > Q.media,
    Q.moda,
    Q.mediana
  )
)
...
Q.analise.cor.Kurtosis <- ifelse(
  Q.conclusao.Kurtosis == Mesokurtic,
  "ok",
  "nao ok"
)
...
Q.conclusao.assimetria.1 <- ifelse(
  Q.ordem.pos.assimetria == "ok" & Q.ordem.neg.assimetria == "nao ok",
  "Empirical relationship between Mean, Median, and Mode is: Mean > Median > Mode.",
  ifelse(
    Q.ordem.pos.assimetria == "nao ok" & Q.ordem.neg.assimetria == "ok",
    "Empirical relationship between Mean, Median and Mode: Mode > Median > Mean.",
    ""
  )
)
...
Q.assimetria <- signif(
  round(skewness(dataset.truncado$dataset.truncado), digits = 2),
  4
)


Q.magnitude.assimetria <- ifelse(
  abs(Q.assimetria) > 1,
  "The distribution is highly skewed",
  ifelse(
    abs(Q.assimetria) > 0.5 & abs(Q.assimetria) <= 1,
    "The distribution is moderately skewed",
    ifelse(
      abs(Q.assimetria) > 0.15 & abs(Q.assimetria) <= 0.5,
      "The distribution is slightly skewed",
      "The distribution is approximately symmetric."
    )
  )
...
Q.conclusao.assimetria.2 <- paste(
  Q.magnitude.assimetria,
  Q.sinal.assimetria
)


Q.conclusao.assimetria <- paste(
  Q.conclusao.assimetria.1,
  Q.conclusao.assimetria.2
)


Q.analise.cor.assimetria <- ifelse(
  Q.magnitude.assimetria == "The distribution is highly skewed",
  "nao ok",
  "ok"
)
...
Mistura.Normal.Nomes <- c(
  "Global Midpoint (Mode or Median)",
  "Cluster Mean",
  "Cluster SD",
...
Mistura.Normal.Cluster.1 <- c(
  round(Q.midpoint, digits = ajuste.casa.decimal),
...
Table.12.data.frame <- data.frame(
  Mistura.Normal.Nomes,
...
titulo.tabela.12 <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 12.</strong> Cluster-specific statistical parameters estimated by maximum likelihood (\u03BB<sub>LabRI</sub> = 1; n<sub>New1</sub> =",
  n.after.truncation,
  ").</div>"
)


titulo.tabela.12.transf <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 12.</strong> Cluster-specific statistical parameters estimated by maximum likelihood (",
  lambda.method,
  "; n<sub>New1</sub> =",
  n.after.truncation,
  ").</div>"
...
Table.12 <- kbl(
  ...
  footnote(
    general_title = "<div style='text-align: justify; color: black;'>Footnote</div>",
    general = footnote_text.Table.12,
    escape = FALSE
  )
...
fit.1.his <- list(
  ...
  sigma = sigma.hist,
  lambda = lambda.hist,
  posterior = posterior.hist,
  x = x.hist
)


##### 3.2.18.1. OPTIMIZATION: free posterior matrix from fit.1 #################
...
txt.Components <- ifelse(
  isTRUE(n.misturas == "one"),
  "Component",
  "Components"
)
...
plot.hist.mix <- function(EM, family = "normal") {
  ...
  pars <- with(
    EM,
    data.frame(comp = colnames(posterior), mu, sigma, lambda)
  )
...
titulo.fig.17 <- paste(
  "Fig. 17. Histogram and 'Truncation band':",
  range.truncado.clusters
)
...
Fig.17 <- hist.mix +
  ...
  scale_linetype_manual(
    values = c("Truncation band" = "dashed"),
    name = ""
  )
...
titulo.fig.18 <- paste(
  "Fig.18. Density plot and Box plot of the data from the 'Truncation band' (\u03BB<sub>LabRI</sub> = 1; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ").",
  sep = ""
)
...
titulo.fig.18.tranf <- paste(
  "Fig.18. Density plot and Box plot of the data from the 'Truncation band' (",
  lambda.method,
  "; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ")",
...
Q.g1 <- qplot(
  x = "",
  y = dataset.truncado$dataset.truncado,
...
Q.g2 <- qplot(
  x = dataset.truncado$dataset.truncado,
  data = data.frame(dataset.truncado$dataset.truncado),
  geom = "density"
) +
  labs(
  ...
  geom_density(color = "sienna3", alpha = 0.6, fill = "sienna1") +
  stat_function(
    fun = function(x) dnorm(x, mean = Q.media, sd = Q.dp),
    color = "black",
    size = 1.2
  ) +
  geom_vline(
    aes(xintercept = Q.moda, colour = Q.moda.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  geom_vline(
    aes(xintercept = Q.mediana, colour = Q.mediana.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  geom_vline(
    aes(xintercept = Q.media, colour = Q.media.texto),
    size = 1.2,
    linetype = "solid"
  ) +
  scale_colour_manual(
    "",
    breaks = c(Q.moda.texto, Q.mediana.texto, Q.media.texto),
...
titulo.fig.19 <- paste(
  "Fig.19. Ogive graph of the 'Truncation band' (\u03BB<sub>LabRI</sub> = 1; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ").",
  sep = ""
)
...
titulo.fig.19.tranf <- paste(
  "Fig.19. Ogive graph of the 'Truncation band' (",
  lambda.method,
  "; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ")",
...
titulo.fig.20 <- paste(
  "Fig.20. Q-Q Plot of the 'Truncation band' (\u03BB<sub>LabRI</sub> = 1; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ").",
  sep = ""
)
...
titulo.fig.20.tranf <- paste(
  "Fig.20. Q-Q Plot of the 'Truncation band' (",
  lambda.method,
  "; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ")",
...
Fig.20 <- ggplot(
  data.frame(dataset.truncado$dataset.truncado),
...
Titulo.Tabela.13 <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 13.</strong> Q-Q Plot of the 'Truncation band' (\u03BB<sub>LabRI</sub> = 1; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ").</div>",
  sep = ""
)

Titulo.Tabela.13.tranf <- paste(
  "<div style='text-align: justify; color: black;'><strong>Table 13.</strong> Q-Q Plot of the 'Truncation band' (",
  lambda.method,
  "; n<sub>New2</sub> = ",
  n.truncado.clusters,
  ")</div>",
...
Table.13 <- kbl(
  ...
  footnote(
    general_title = "<div style='text-align: justify; color: black;'>Footnote</div>",
    general = footnote_text.Tables.13,
    escape = FALSE
  )
...
URL.RI.1.cluster.back <- round(
  InvBoxCox(LS.IR.Cluster.1, lambda.LabRI),
  digits = casa.decimal
)

URL.RI.1.cluster <- ifelse(
  needs_log_transform == TRUE,
  URL.RI.1.cluster.back,
  LS.IR.Cluster.1
)

URL.RI.LabRI.1.cluster <- round(
  ifelse(n.clusters.habili == 1, URL.RI.1.cluster, NA),
  casa.decimal2
)


LRL.RI.1.cluster.back <- round(
  InvBoxCox(LI.IR.Cluster.1, lambda.LabRI),
  digits = casa.decimal
)

LRL.RI.1.cluster <- ifelse(
  needs_log_transform == TRUE,
  LRL.RI.1.cluster.back,
  LI.IR.Cluster.1
)

LRL.RI.LabRI.1.cluster <- round(
  ifelse(n.clusters.habili == 1, LRL.RI.1.cluster, NA),
  casa.decimal2
)


IR.LabRI.1.cluster <- paste(
  round(LRL.RI.LabRI.1.cluster, digits = casa.decimal),
  "to",
...
Table.14.data.frame <- data.frame(
  ifelse(isTRUE(use.np.method),
...
')
  ...
  Table.15 <- kbl(
    ...
    add_header_above(
      c(" " = 1, "Before Winsorization" = 2, "After Winsorization" = 3),
      bold = TRUE,
      color = "White",
      background = "#0d47a1"
    ) |>
  ...
  identify_main_cluster_and_RI <- function(num_clusters,
    ...
    lower_limit <- midpoints[min_distance_cluster] -
      1.96 * sds[min_distance_cluster]
    
    upper_limit <- midpoints[min_distance_cluster] +
      1.96 * sds[min_distance_cluster]
  ...
  vector.clusters <- function(k.new, vetor.Mi, vetor.Sigma, vetor.Weights) {
    ...
    return(list(
      Mi = vetor.Mi,
      Sigma = vetor.Sigma,
      Weights = vetor.Weights
    ))
  }
  
  
  results.vector.clusters <- vector.clusters(
    k.new = k.new,
    vetor.Mi = c(Mi.1, Mi.2, Mi.3, Mi.4, Mi.5, Mi.6, Mi.7),
    vetor.Sigma = c(Sigma.1, Sigma.2, Sigma.3, Sigma.4, Sigma.5, Sigma.6, Sigma.7),
    vetor.Weights = c(Weights.1, Weights.2, Weights.3, Weights.4, Weights.5, Weights.6, Weights.7)
  )
  ...
  isolated.cluster <- identify_main_cluster_and_RI(
    num_clusters = k.new,
    midpoints = vector.Mi,
    sds = vector.Sigma,
  ...
  outside.reflimR.2 <- percentage_outside_RI(
  ...
  outside.refineR.2 <- percentage_outside_RI(
    dataset = dataset_orig_truncado$dataset_orig_truncado,
  ...
  outside.refineR.2 <- c(
    outside.refineR.2$percentual_abaixo,
    outside.refineR.2$percentual_acima
  )
  
  outside.reflimR.2 <- c(
    outside.reflimR.2$percentual_abaixo,
      ...
      return(list(
        URL = round(
          quantile(
            ...
            ifelse(isTRUE(lado == "Right_sided"), 0.95, 0.975)
          ),
          digits = 5
        ),
        LRL = round(
          quantile(
            dataset.truncado$dataset.truncado,
            ifelse(isTRUE(lado == "Right_sided"), 0.000, 0.025)
          ),
          digits = 5
        )
      ))
  ...
  RI.percentil <- check_all_values(
    outside1 = outside.refineR.2,
  ...
  LRL.percentil <- round(
    ifelse(
      needs_log_transform == TRUE,
      InvBoxCox(LRL.percentil.1, lambda.LabRI),
      LRL.percentil.1
    ),
    digits = casa.decimal2
  )
  ...
  URL.percentil <- round(
    ifelse(
      needs_log_transform == TRUE,
      InvBoxCox(URL.percentil.1, lambda.LabRI),
      URL.percentil.1
    ),
    digits = casa.decimal2
  )
  ...
  URL.isolated.percentil <- ifelse(
    is.na(URL.isolated.cluster),
    ifelse(is.na(URL.percentil), NA, URL.percentil),
    URL.isolated.cluster
  )
  
  LRL.isolated.percentil <- ifelse(
    is.na(LRL.isolated.cluster),
    ifelse(is.na(LRL.percentil), NA, LRL.percentil),
    LRL.isolated.cluster
  )
...
')
  ...
  LTL_LRL.Haeckel_approach1 <- ifelse(
    LTL_LRL.Haeckel_approach1 < 0,
    0,
  ...
  LTL_URL.Haeckel_approach1 <- ifelse(
    LTL_URL.Haeckel_approach1 < 0,
    0,
  ...
  Two.stage_Winso <- function(LTL_LRL.Haeckel_approach1,
    ...
    LRL.faixa.windsor.Haeckel_approach1 <- c(
      LTL_LRL.Haeckel_approach1,
      UTL_LRL.Haeckel_approach1
    )
    
    URL.faixa.windsor.Haeckel_approach1 <- c(
      LTL_URL.Haeckel_approach1,
      UTL_URL.Haeckel_approach1
    )
    ...
    lista.URL.wind <- winsorize(
      lista.URL,
      threshold = URL.faixa.windsor.Haeckel_approach1,
      method = "raw"
    )
    
    lista.URL.wind.2 <- winsorize(
      lista.URL.wind,
      threshold = 1,
      method = "zscore"
    )
    
    lista.LRL.wind <- winsorize(
      lista.LRL,
      threshold = LRL.faixa.windsor.Haeckel_approach1,
      method = "raw"
    )
    
    lista.LRL.wind.2 <- winsorize(
      lista.LRL.wind,
      threshold = 0.5,
      robust = TRUE,
      method = "zscore"
    ...
    result <- list(
      List.LRL.Winsorized = lista.LRL.wind.2,
  ...
  Winsorization_RL <- Two.stage_Winso(
    LTL_LRL.Haeckel_approach1 = LTL_LRL.Haeckel_approach1,
  ...
  if (is.na(ifelse(k.new == 1, NA, LRL.isolated.percentil)) &&
    ...
    get_kmeans_limits_and_plot <- function(LRL.RI.refineR.cenario.1,
      ...
      x_limits <- range(
        ...
        limite_inferior_escolhido,
        na.rm = TRUE
      )
      
      y_limits <- range(
        limites$Limite_superior,
        limite_superior_escolhido,
        na.rm = TRUE
      )
  ...
  URL.RI.refineR.reflimR.kmeans <- round(
    refineR.reflim.kmeans$limite_superior_escolhido,
    digits = casa.decimal2
  )
  
  LRL.RI.refineR.reflimR.kmeans <- round(
    refineR.reflim.kmeans$limite_inferior_escolhido,
    digits = casa.decimal2
  )
  
  LRL.RI.refineR.reflimR.kmeans <- ifelse(
    LRL.RI.refineR.reflimR.kmeans < 0,
    0,
    LRL.RI.refineR.reflimR.kmeans
  )
  
  RI.refineR.reflimR.kmeans <- paste(
    LRL.RI.refineR.reflimR.kmeans,
    " to ",
  ...
  IR.refineR.cenario.1.wind <- paste(
    round(lista.LRL.wind.2[1], casa.decimal2),
    " to ",
    round(lista.URL.wind.2[1], casa.decimal2)
  )
  
  IR.refineR.cenario.2.wind <- paste(
    round(lista.LRL.wind.2[3], casa.decimal2),
    " to ",
    round(lista.URL.wind.2[3], casa.decimal2)
  )
  
  IR.reflimR.cenario.1.wind <- paste(
    round(lista.LRL.wind.2[2], casa.decimal2),
    " to ",
    round(lista.URL.wind.2[2], casa.decimal2)
  )
  
  IR.reflimR.cenario.2.wind <- paste(
    round(lista.LRL.wind.2[4], casa.decimal2),
    " to ",
    round(lista.URL.wind.2[4], casa.decimal2)
  )
  
  
  IR.EM.Algorithm <- ifelse(
    k.new == 1,
    "Option disabled",
    ifelse(
      is.na(LRL.isolated.percentil) == TRUE &
        is.na(URL.isolated.percentil) == TRUE,
      "Option disabled",
      paste(
        round(LRL.isolated.percentil, casa.decimal2),
        " to ",
        round(URL.isolated.percentil, casa.decimal2)
      )
    )
  )
  
  
  IR.EM.Algorithm.wind <- ifelse(
    k.new == 1,
    "Option disabled",
    ifelse(
      is.na(LRL.isolated.percentil) == TRUE &
      ...
      "Option disabled",
      paste(
        round(lista.LRL.wind.2[5], casa.decimal2),
        " to ",
        round(lista.URL.wind.2[5], casa.decimal2)
      )
    )
  )
  ...
  lista.RIs.before.before.windsor <- c(
    IR.refineR.reflimR.NP.fallback.cenario.1,
    IR.reflimR.cenario.1,
    " --- "
  )
  
  lista.RIs.after.before.windsor <- c(
    IR.refineR.reflimR.NP.fallback.cenario.2,
    IR.reflimR.cenario.2,
    IR.EM.Algorithm
  )
  
  lista.RIs.before.after.windsor <- c(
    IR.refineR.cenario.1.wind,
    IR.reflimR.cenario.1.wind,
    " --- "
  )
  
  lista.RIs.after.after.windsor <- c(
    IR.refineR.cenario.2.wind,
...
val.lambda.refineR.1 <- if (is.na(val.LRL.RI.refineR.cenario.1) &&
                            is.na(val.URL.RI.refineR.cenario.1)) {
  "NA"
} else {
  lambda.refineR.1
}

val.lambda.refineR.2 <- if (is.na(val.LRL.RI.refineR.cenario.2) &&
                            is.na(val.URL.RI.refineR.cenario.2)) {
  "NA"
} else {
  lambda.refineR.2
}
...
val.lambda.reflimR.1 <- if (is.na(val.LRL.RI.reflimR.cenario.1) &&
                            is.na(val.URL.RI.reflimR.cenario.1)) {
  "NA"
} else {
  lambda.reflimR.1
}

val.lambda.reflimR.2 <- if (is.na(val.LRL.RI.reflimR.cenario.2) &&
                            is.na(val.URL.RI.reflimR.cenario.2)) {
  "NA"
} else {
  lambda.reflimR.2
}
...
###### 3.3.4.4.2.3. Reservoir for combined
...
###### 3.3.4.4.3. CI for each confidence level
...
plot_UL_IE <- ggplot() +
  geom_rect(
    aes(
      xmin = URL_LL_Equivalence_Interval,
      xmax = URL_UL_Equivalence_Interval,
      ymin = -Inf,
      ymax = Inf,
      fill = "Equivalent Region"
    ),
    alpha = 0.5
  ) +
  geom_rect(
    aes(
      ...
      ymin = -Inf,
      ymax = Inf,
      fill = "Not Equivalent Region"
    ),
    alpha = 0.2
  ) +
  geom_rect(
    aes(
      xmin = URL_UL_Equivalence_Interval,
      xmax = Inf,
      ymin = -Inf,
      ymax = Inf,
      fill = "Not Equivalent Region"
...
plot_LL_IE <- if (isTRUE(lado == "Right_sided")) {
  ...
  ggplot() +
    geom_rect(
      aes(
        xmin = LRL_LL_Equivalence_Interval,
    ...
    geom_rect(
      aes(
        xmin = LRL_UL_Equivalence_Interval,
        xmax = Inf,
        ymin = -Inf,
        ymax = Inf,
        fill = "Not Equivalent Region"
    ...
    labs(
      x = "Difference Between LRLs (or Effect Size<sub>LRLs</sub>)",
      y = ""
    ) +
    scale_x_continuous(breaks = pretty_breaks(n = 9)) +
    guides(
